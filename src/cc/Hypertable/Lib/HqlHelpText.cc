...
namespace {
  ...
  const char *help_text_load_data_infile[] = {
    ...
    "PARALLEL = n",
    "",
    "The PARALLEL option provides a way to read the input file with n different",
    "cursors, equally spaced throughout the file and returns cells from the",
    "cursors in round-robin fashion until all the cells from the file are",
    "loaded. This avoids the problem with loading backups, because the keys",
    "being inserted are not always at the very end, they're evenly spread across",
    "the whole key space. This feature doesn't work for compressed input files",
    "or data that is being read from stdin.",
    "",
    "NOESCAPE",
    "",
